...
void* CentralCache::AllocateBatch(size_t size, size_t batch_nums){
    ...
    std::lock_guard<std::mutex> lock(mutexes_[index]); //加锁

    size_t remain = batch_nums;
    void* head = nullptr;
    void** tail = &head;
    ...
    while(remain > 0){
        ...
        if(!span){
            ...
            //分割Span为小块，构建空闲链表
            void* start = reinterpret_cast<void*>(newSpan->startAddr);
            void* end = start + newSpan->page_count * PageCache::GetInstance().GetPageSize();
            newSpan->free_list = start;
            void* cur = start;
            while(cur + align_size < end){
                *reinterpret_cast<void**>(cur) = cur + align_size;
                cur += align_size;
            }
        ...
        void* end = start;
        size_t actual_num = std::min(remain, span->free_count);
        ...
        *reinterpret_cast<void**>(end) = nullptr;
        *tail = start;
        while(*tail) tail = reinterpret_cast<void**>(*tail);
        remain -= actual_num;
    }
    return head;
}
    ...
    while(node && n < count){
        ...
        if(span){
            *reinterpret_cast<void**>(node) = span->free_list;
            ...
            size_t total_blocks = (span->page_count * PageCache::GetInstance().GetPageSize()) / span->block_size;
